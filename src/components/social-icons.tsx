...
export default function SocialIcons() {
  ...
  return (
    <>
      ...
      <div className="fixed top-1/4 right-1 p-1.5 gap-2 z-40 backdrop-blur-sm rounded-2xl bg-black/5 hidden sm:grid">
        {socialLinks.map((link, index) => (
          <a href={link.link} key={index} className="social-icon-link" target="_blank" rel="noreferrer">
            <svg
              className={`${link.title} overflow-visible block w-11 rounded-lg p-2 duration-200`}
      ...
      {isMobile && (
        <div
          className={`fixed bottom-0 left-0 right-0 flex justify-center gap-4 p-2 z-40 backdrop-blur-sm bg-black/5 sm:hidden mobile-social-container ${
        ...
        >
          {socialLinks.map((link, index) => (
            ...
            >
              <svg
                className={`${link.title} overflow-visible block w-8 rounded-lg p-2 duration-200`}
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                {link.icon}
