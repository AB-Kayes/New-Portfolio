...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return (
    <main className="container mx-auto px-4 py-20 max-w-11/12">
      ...
      <div className="grid gap-8 md:grid-cols-[2fr_1.5fr]">
        <div className="relative px-0 sm:px-6">
          <Carousel className="w-full">
            ...
            {project.imgCount > 1 && (
              <>
                <CarouselPrevious className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 bg-background/60 hover:bg-background/80 backdrop-blur-sm -left-3 sm:-left-5 absolute border-primary/20" />
        ...
        <div>
          <h1 className="mb-4 text-2xl md:text-3xl font-bold">{project.title}</h1>
      ...
      {project.features && (
        <div className="mt-12">
          <h2 className="mb-6 text-xl md:text-2xl font-bold">Key Features</h2>
          <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {project.features.map((feature, index) => (
              <li key={index} className="rounded-lg border border-gray-300 p-4 hover:shadow-md transition-shadow">
